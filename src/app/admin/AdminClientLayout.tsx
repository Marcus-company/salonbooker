...
export default function AdminClientLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row">
      {/* Mobile Header */}
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 z-50 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">SalonBooker</h1>
            {userRole === 'admin' && (
      ...
      {menuOpen && (
        <>
          ...
          />
          <div className="md:hidden fixed top-16 left-0 right-0 bottom-0 bg-white dark:bg-slate-900 z-40 overflow-auto">
            <nav className="p-4 space-y-1">
              {navItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-3 px-4 py-3 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                >
                  <span>{item.icon}</span>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700 mt-4 space-y-3">
              <div className="flex items-center gap-3">
      ...
      <aside className="hidden md:flex w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex-col">
        <div className="p-6 border-b border-slate-200 dark:border-slate-700">
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">SalonBooker</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">HairsalonX Admin</p>
          {userRole === 'admin' && (
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Thema</span>
            <ThemeToggle />
          </div>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-lg">
      ...
      {/* Main content */}
      <main className="flex-1 overflow-auto pt-16 md:pt-0 bg-slate-50 dark:bg-slate-950">
        {children}
      </main>
